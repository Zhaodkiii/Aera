...
@MainActor
final class CaseListVM: ObservableObject {
    @Published var items: [CaseDTO] = []
    @Published var searchText = ""
    @Published var ordering = "-visit_date"
    ...
    func loadMore() async {
        ...
        do {
            let resp = try await APIClient.shared.fetchCases(
                search: searchText.isEmpty ? nil : searchText,
                ordering: ordering,
                page: page,
                pageSize: pageSize
            )
            if page == 1 { items = resp.results } else { items += resp.results }
            page += 1
            hasMore = (items.count < resp.count)
        } catch {
...
//                    // 简单示例：自动登录（生产环境请走登录页）
//                    try? await APIClient.shared.login(username: "admin", password: "yourpass")
//                }
